...
class LogFile
{
	...
	// This is called before a new instruction executes.
	void new_insn(U4 cs, U4 ip)
	{
		++insn_count;
	...
	void write_physical(U4 linear, U1 *data, U4 dsize, char type)
	{
		if(cache.linear + cache.size == linear && cache.size + dsize <= 65536 && cache.type == type)
			;
		else
		{
			sync();
		...
		for(U4 i = 0; i < dsize; ++i)
		{
			cache.data[cache.size] = data[i];
			++cache.size;
		}
	}
	...
	void write_again()
	{
		...
		s.type = 0xa;
		s.size = 4;
		sync();					// write out any MOVS/STOS data before each iteration
		fwrite(&s, 4, 1, f);
	}
	
	void write_gen_reg(U4 reg, U4 value)
	{
		struct
		{
			U4 type : 8;
	...
	LogFile()
	{
		...
		s.type = 255;// start
		s.size = 4;	// size of start
		fwrite(&s, 4, 1, f);
		cache.linear = 0;
		cache.size = 0;
		cache.type = '\0';	// invalid
		insn_count = 0;
	}
	...
	~LogFile()
	{
		...
		s.type = 0;	// end of file
		s.size = 4 + 4;
		fwrite(&s, 4, 1, f);
		fwrite(&insn_count, 4, 1, f);		// must be last dword in file
		fclose(f);
	}
	...
	void sync()
	{
		if(cache.size != 0)
		{
			...
			s.type = 2;		// mem write
			s.size = cache.size + 4 + 4 + 1;
			fwrite(&s, 4, 1, f);
			fwrite(&cache.linear, 4, 1, f);
			fwrite(&cache.type, 1, 1, f);
			fwrite(cache.data, cache.size, 1, f);
			cache.size = 0;
			cache.type = '\0';	// invalid
		}
	}
	...
	public LinearMemory
{
	...
	{
		log.write_physical(linear, data, dsize, type);
...
U8 old_rip = ~0ULL;
U8 sequence = 0;
U4 old_regs[8], ii, old_sr[6], old_efl;

	if(argc > 1)
	{
		printf("Using boot sector: %s\n", argv[1]);
		unsigned char boot[512];
		FILE *fi = fopen(argv[1], "rb");
		if(fi == NULL)
		{
			printf("Unable to open boot sector.\n");
			return 1;
		}
		if(fread(boot, 512, 1, fi) != 1)
		{
			printf("Unable to read from file.\n");
			return 1;
		}
		fclose(fi);
		pc.linmem->write_linear(0x7c00, boot, 512, 'n');
	}

#if 0
	// begin load hack
	//system.setup_test("test.img", 0x8000, 0x0000, 0x1000);
	FILE *fi = fopen("emudos.bin", "rb");
	if(fi == NULL)
	{
		std::cerr << "Unable to open file: emudos.bin" << std::endl;
		return 1;
	}
	fseek(fi, -1, SEEK_END);
	long fs = 1 + ftell(fi);
	rewind(fi);
	unsigned char *fd = new unsigned char [fs];
	if(fread(fd, fs, 1, fi) != 1)
	{
		std::cerr << "Unable to read file: emudos.bin" << std::endl;
		fclose(fi);
		return 1;
	}
	fclose(fi);
	pc.linmem->write_linear(0x50000, fd, fs, 'n');
	delete [] fd;

	unsigned char q[5] = {0xea, 0, 0, 0, 0x50};
	pc.linmem->write_linear(0x7c00, q, 5, 'n');
	// end load hack
#endif
...
//std::cout << sequence << std::endl;
			...
			U2 csT = pc.exec->sregs[rCS].value;
			U2 ipT = pc.exec->rip;
			log.new_insn(csT, ipT);
...
old_rip = pc.exec->rip;
			...
			if(pc.handle_intr())
			{
				...
				for(ii = 0; ii < 8; ++ii)
					log.write_gen_reg(ii, pc.exec->regs[ii].u.ivalue[0]);
				for(ii = 0; ii < 6; ++ii)
					log.write_gen_reg(0x10 + ii, pc.exec->sregs[ii].value);
				log.write_gen_reg(0x1f, pc.exec->rflags);

				// returned to a new instruction.
				U2 csT = pc.exec->sregs[rCS].value;
				U2 ipT = pc.exec->rip;
				log.new_insn(csT, ipT);
			}
...
tmpofs = pc.exec->rip;
			...
			{
				for(ii = 0; ii < 8; ++ii)
					old_regs[ii] = pc.exec->regs[ii].u.ivalue[0];
				old_efl = pc.exec->rflags;
				for(ii = 0; ii < 6; ++ii)
					old_sr[ii] = pc.exec->sregs[ii].value;
...
#endif
if(argc > 2) {
printf("%04x:%04x ", (int)(csT), (int)(ipT));
dest[0] = '\0';
disasm.disasm(dest, &pc.decode, NULL, NULL, tmpofs + pc.exec->size);
printf("%s\n", dest);
}
				...
				for(ii = 0; ii < 8; ++ii)
				{
					if(old_regs[ii] != pc.exec->regs[ii].u.ivalue[0])
						log.write_gen_reg(ii, pc.exec->regs[ii].u.ivalue[0]);
				}
				for(ii = 0; ii < 6; ++ii)
				{
					if(old_sr[ii] != pc.exec->sregs[ii].value)
						log.write_gen_reg(0x10 + ii, pc.exec->sregs[ii].value);
				}
				log.write_gen_reg(0x1f, pc.exec->rflags);
