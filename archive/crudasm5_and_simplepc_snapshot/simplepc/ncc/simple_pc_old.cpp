...
#if DISASM
if(pc.exec->rip == 0x7cde)
{
	std::cout << "ds:si=";
	for(int i = 0; i < 11; ++i)
	{
		std::cout << pc.ram[pc.exec->regs[rSI].u.svalue[0] + i + (U4)(pc.exec->sregs[rDS].value) * 16];
	}
	std::cout << "; es(" << std::hex << pc.exec->sregs[rES].value << ")" << std::dec << ":di(" << std::hex << pc.exec->regs[rDI].u.svalue[0] << std::dec << ")=";
	for(int i = 0; i < 11; ++i)
	{
		std::cout << pc.ram[pc.exec->regs[rDI].u.svalue[0] + i + (U4)(pc.exec->sregs[rES].value) * 16];
	}
	std::cout << std::endl;
}
#endif
...
#if DISASM
///if(pc.exec->sregs[rCS].value == 0x1fe0 && pc.exec->rip >= 0x7d05)
{
	std::cout << std::hex << "RAX=" << pc.exec->regs[rAX].u.value << std::dec << "  ";
	std::cout << std::hex << "RSI=" << pc.exec->regs[rSI].u.value << std::dec << "  ";
	std::cout << std::hex << "RDI=" << pc.exec->regs[rDI].u.value << std::dec << "  ";
}

std::cout << std::hex << pc.exec->sregs[rCS].value << ":" << pc.exec->rip << std::dec << "  ";
std::cout << "; ss(" << std::hex << pc.exec->sregs[rSS].value << ")" << std::dec << ":sp(" << std::hex << pc.exec->regs[rSP].u.svalue[0] << std::dec << ") ";
std::cout << std::hex << "CS=" << pc.exec->sregs[rCS].value << std::dec << ",";
std::cout << std::hex << "IP=" << pc.exec->rip << std::dec << "  ";
std::cout << std::hex << "DS=" << pc.exec->sregs[rDS].value << std::dec << "  ";
std::cout << std::hex << "ES=" << pc.exec->sregs[0].value << std::dec << "  ";
std::cout << std::hex << "SS=" << pc.exec->sregs[rSS].value << std::dec << "  ";
U8 xxoffset, xxvalue;
xxoffset = pc.exec->regs[rBP].u.svalue[0]+0x0e;
std::cout << std::hex << "BP+0x0e=" << (U2)(xxoffset) << std::dec << "  ";

/*std::cout << std::hex << "seg=";
xxoffset = (U4)(pc.exec->sregs[rSS].value) * 16 + 0x7c5a;
xxvalue = *(U2 *)(pc.ram + xxoffset + 2);
std::cout << xxvalue << std::dec << "  ";*/
std::cout << std::hex << "value=";
xxoffset = (U4)(pc.exec->sregs[rSS].value) * 16 + xxoffset;
xxvalue = *(U2 *)(pc.ram + xxoffset);
std::cout << xxvalue << std::dec << "  ";
...
#if DISASM
if(pc.decode.insn[0] == 0 && pc.decode.insn[1] == 0)
{
	std::cout << "Possible Error case 1234." << std::endl;
	throw -1;
}
...
#if 0
		if(pc.decode.icode->insn == insn_call)
		{
			std::cout << "CALL FROM " << std::hex << tmpofs + pc.exec->size << std::dec << std::endl;
		}
		else
		if(pc.decode.icode->insn == insn__ret || pc.decode.icode->insn == insn__retnum)
		{
			std::cout << "RETURN TO " << std::hex << pc.exec->rip << std::dec << std::endl;
		}
std::cout << std::hex << tmpseg << std::dec << ":";
std::cout << std::hex << tmpofs << std::dec << "  ";
U4 zofs = 16 * (U4)(pc.exec->sregs[rDS].value) + (U4)(pc.exec->regs[rSI].u.svalue[0] + 0x18);
U2 zval = *(U2 *)(pc.ram + zofs);
std::cout << std::hex << zval << std::dec << "  ";
std::cout << std::hex << zofs << std::dec << "  ";
std::cout << (sequence) << "  ";
dest[0] = '\0';
disasm.disasm(dest, &pc.decode, NULL, NULL, tmpofs + pc.exec->size);
std::cout << dest << std::endl;
