...
emu {
	assert(_lognot(_zero(arg)));		// else division by zero exception
	B1 tmp = AL;
	AH = udiv(tmp, arg);
	AL = umod(tmp, arg);
	OF = undefined;
...
emu {
	...
	OF = _quest(_zero(tmpcount), OF, _quest(_eq(count, tr<B1>(1)), _logxor(sign(dest), CF), undefined));
}
...
"c8 imm,imm"
emu {
	assert(eq(y, tr<B1>(0)));	// else unimplemented!
	s16: push(BP);
	s16: BP = SP;
	s16: reserve(x);
	s32: push(EBP);
...
emu {
	o16: DX = sub(tr<B2>(0), zx<B2>(sign(AX)));
	o32: EDX = sub(tr<B4>(0), zx<B4>(sign(EAX)));
	o64: RDX = sub(tr<B8>(0), zx<B8>(sign(RAX)));
}
...
"ce void"						! no64
emu {
	assert(_lognot(OF));				// else generate interrupt
}
...
"f7 /5 rm"							! P=osz
emu {
	o16: BIT tmp = _smul_overflow(AX, arg);
	o16: B4 result = cmul(sx<B4>(AX), sx<B4>(arg));
	o16: AX = tr<B2>(result);
	o16: DX = tr<B2>(shru_(result, tr<B1>(16)));

	o32: BIT tmp = _smul_overflow(EAX, arg);
	o32: B8 result = cmul(sx<B8>(EAX), sx<B8>(arg));
	o32: EAX = tr<B4>(result);
	o32: EDX = tr<B4>(shru_(result, tr<B1>(32)));

	o64: BIT tmp = _smul_overflow(RAX, arg);
	o64: B16 result = cmul(sx<B16>(RAX), sx<B16>(arg));
	o64: RAX = tr<B8>(result);
	o64: RDX = tr<B8>(shru_(result, tr<B1>(64)));
...
"f7 /4 rm"							! P=osz
emu {
	o16: B4 result = cmul(zx<B4>(AX), zx<B4>(arg));
	o16: AX = tr<B2>(result);
	o16: DX = tr<B2>(shru_(result, tr<B1>(16)));
	o16: BIT tmp = _umul_overflow(AX, arg);

	o32: B8 result = cmul(zx<B8>(EAX), zx<B8>(arg));
	o32: EAX = tr<B4>(result);
	o32: EDX = tr<B4>(shru_(result, tr<B1>(32)));
	o32: BIT tmp = _umul_overflow(EAX, arg);

	o64: B16 result = cmul(zx<B16>(RAX), zx<B16>(arg));
	o64: RAX = tr<B8>(result);
	o64: RDX = tr<B8>(shru_(result, tr<B1>(64)));
	o64: BIT tmp = _umul_overflow(RAX, arg);
...
emu {
	...
	o16: B2 s = tr<B2>(q);
	o16: assert(eq(zx<B4>(s), q));		// make sure it fits
	o16: AX = s;
	o16: DX = tr<B2>(r);
	...
	o32: B4 s = tr<B4>(q);
	o32: assert(eq(zx<B8>(s), q));		// make sure it fits
	o32: EAX = s;
	o32: EDX = tr<B4>(r);
	...
	o64: B8 s = tr<B8>(q);
	o64: assert(eq(zx<B16>(s), q));		// make sure it fits
	o64: RAX = s;
	o64: RDX = tr<B8>(r);
...
emu {
	...
	o16: B2 s = tr<B2>(q);
	o16: assert(eq(sx<B4>(s), q));		// make sure it fits
	o16: AX = s;
	o16: DX = tr<B2>(r);
	...
	o32: B4 s = tr<B4>(q);
	o32: assert(eq(sx<B8>(s), q));		// make sure it fits
	o32: EAX = s;
	o32: EDX = tr<B4>(r);
	...
	o64: B8 s = tr<B8>(q);
	o64: assert(eq(sx<B16>(s), q));		// make sure it fits
	o64: RAX = s;
	o64: RDX = tr<B8>(r);
